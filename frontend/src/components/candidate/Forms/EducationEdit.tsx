...
export default function EditEducationForm({selectedEducation, editEducationModalOpen, closeEditEducationModal, token} : any) {
    ...
    useEffect(() => {
        ...
        setEducationLevel(selectedEducation.level)
        setEducationStream(selectedEducation.stream)
        setEducationInstitution(selectedEducation.organization)
    ...
    function validateEducation(): boolean {
        ...
        startdateerror ? setStartYearError('Provide a valid date') : setStartYearError('')
        enddaterror ? setEndYearError('Provide a valid date') : setEndYearError('')
    ...
    async function editEducation() {
        const isValidated = validateEducation()
        ...
        <Box sx={style}>
          ...
          <Box sx={{width:'100%', display:'flex', justifyContent:'between', gap:'100px'}}>
            <div className="w-full">
                ...
                <input value={startYear} onChange={(event) => setStartYear(event.target.value)} type="text" name="name" id="name" className="outline-none border border-gray-400 w-full mt-2 block p-2 rounded" />
                <label className='error-label'>{startYearError}</label>
            </div>

            <div className="w-full">
                <label htmlFor="">End Date</label>
                <input disabled={isPresent ? true : false} value={endYear} onChange={(event) => setEndYear(event.target.value)} type="text" name="name" id="name" className="outline-none border border-gray-400 w-full mt-2 block p-2 rounded" />
                <label className='error-label'>{endYearError}</label>
            </div>
          </Box>
          ...
          <Box sx={{width:'100%', marginTop:'10px'}}>
            <button onClick={editEducation} className="bg-blue-400 rounded w-full p-1 text-white">Add</button>
          </Box>
        </Box>
      </Modal>
    )
