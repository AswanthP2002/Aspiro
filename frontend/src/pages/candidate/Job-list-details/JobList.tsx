...
import {
  recruiterFetchJobLevelLists,
  recruiterFetchJobTypeLists,
  recruiterFetchWorkModeLists,
    ...
    <>
      <section className="job-listing-container pt-15 lg:pt-0 pb-10">
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm">
          ...
          <div className="px-6 py-4 border-b border-gray-100">
            ...
            <p className="text-sm text-gray-500 mt-0.5">
              Discover roles that match your expertise and career goals
            </p>
          </div>
          ...
          <div className="p-5 bg-gray-50/50">
            ...
            <div className="mt-4 flex flex-wrap gap-3">
              {[
                {
                  label: jobTypeFilter,
                  toggle: toggleJobTypeFilterOpen,
                  isOpen: isJobTypeFilterOpen,
                  options: jobTypeOptions,
                  set: setJobTypeFilter,
                  setOpen: setIsJobTypeFilterOpen,
                },
                {
                  label: jobLevelFilter,
                  toggle: toggleJobLevelFilterOpen,
                  isOpen: isJobLevelFilterOpen,
                  options: jobLevelOptions,
                  set: setJobLevelFilter,
                  setOpen: setIsJobLevelFilterOpen,
                },
                {
                  label: workModeFilter,
                  toggle: toggleWorkModeFilter,
                  isOpen: isWorkModeFilterOpen,
                  options: workModeOptions,
                  set: setWorkModeFilter,
                  setOpen: setIsWorkModeFilterOpen,
                },
              ].map((filter, idx) => (
                <div key={idx} className="relative">
                  ...
                  >
                    ...
                    <BiChevronDown
                      size={16}
                      className={`${filter.isOpen ? 'rotate-180' : ''} transition-transform`}
                    />
                  </button>
                    ...
                    <div className="absolute top-full mt-2 z-30 bg-white w-48 border border-slate-100 rounded-lg shadow-xl py-1 animate-in fade-in zoom-in transition-all duration-300">
                      ...
                      {filter.options.map((option, i) => (
                        <button
                          key={i}
                          onClick={() => {
                            filter.set(option.name as string);
                            filter.setOpen(false);
                          }}
        ...
        <div className="mt-5 grid grid-cols-1 gap-3">
          {jobs.length > 0 &&
            jobs.map((job: LoadJobsForPublicData) => <JobListTile key={job._id} data={job} />)}
        </div>
        {jobs.length > 0 && (
          <div className="flex mt-5 items-center justify-center gap-3">
            <button
              ...
              disabled={page === 1}
              className={`border border-slate-300 rounded-md text-xs font-medium px-3 py-2 ${page === 1 ? 'bg-gray-300 text-gray-400' : 'bg-white'}`}
            >
              Prev
            </button>
            {Array.from(new Array(totalPages)).map((_, index) => (
              <button
                ...
                className={`${page === index + 1 ? 'bg-blue-500 text-white' : 'bg-white border border-slate-300'} px-4 py-1 rounded-md`}
              >
                {index + 1}
              </button>
            ))}
            <button
              ...
              className={`border border-slate-300 rounded-md text-xs font-medium px-3 py-2 ${page >= totalPages ? 'bg-gray-300 text-gray-400' : 'bg-white'}`}
            >
              Next
            </button>
          </div>
