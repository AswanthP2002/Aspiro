...
const EditPlan = () => {
  ...
  const {editablePlanData} = location.state || {}

  const { register, control, handleSubmit, reset, watch, formState: { errors } } = useForm<AddPlanFormData>({
    defaultValues: {
      planName: '',
      planDescription: '',
      badgeIcon: 'Lightning',
  ...
  const onSubmit = async (data) => {
    ...
    const confirmResult = await Swal.fire({
      icon: 'question',
      title: `Update plan ${data.planName}?`,
    ...
    if(!confirmResult.isConfirmed){
      ...
      const result = await toast.promise(
        adminEditPlan(editablePlanData._id, payLoad),
        {
  ...
  useEffect(() => {
    if(editablePlanData){
        reset({
            planName:editablePlanData.name,
            badgeIcon: editablePlanData.badgeIcon,
            planDescription: editablePlanData.description,
            isListed: editablePlanData.isListed,
            connectionRequests: editablePlanData.featuresListed.connectionRequests,
            directMessaging: editablePlanData.featuresListed.directMessaging,
            features: editablePlanData.features,
            interviewPractice: editablePlanData.featuresListed.interviewPractice,
            jobApplications: editablePlanData.featuresListed.jobApplications,
            jobPosts: editablePlanData.featuresListed.jobPosts,
            jobRecommendation: editablePlanData.featuresListed.jobRecommendation,
            monthlyPrice: editablePlanData.monthlyPrice,
            yearlyPrice: editablePlanData.yearlyPrice,
            pushJob: editablePlanData.featuresListed.pushJob,
            recruiterProfile: editablePlanData.featuresListed.recruiterProfile,
            resumeAnalyzer: editablePlanData.featuresListed.resumeAnalyzer,
            resumeBuilder: editablePlanData.featuresListed.resumeBuilder,
            smartFilter: editablePlanData.featuresListed.smartFilter,
            socialFeed: editablePlanData.featuresListed.socialFeed,
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="flex gap-4">
              <button onClick={() => navigate(-1)} type="button" className="flex-1 py-3 border border-gray-200 rounded-lg font-bold text-gray-600 bg-white hover:bg-gray-50 transition">
                Cancel
              </button>
