...
import IUserLoginUseCase from '../../interfaces/usecases/user/IUserLogin.usecase';
...
export class UserLoginUseCase implements IUserLoginUseCase {
  ...
  async execute(loginDto: UserLoginInpDTO): Promise<UserLoginOutpDTO> {
    ...
    return {
      token,
      refreshToken,
      user: { email: user.email, id: user._id?.toString() as string },
      role: primaryRole,
    };
  }
}
