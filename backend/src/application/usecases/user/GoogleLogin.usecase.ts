...
export default class GoogleLoginUseCase implements IGoogleLoginUseCase {
  ...
  async execute(googleToken: string): Promise<UserLoginResponseDto> {
    ...
    let createdUser;
    if (!isUserExist) {
      createdUser = await this._userRepo.create({ name, email, googleId, isVerified: true });
    }

    if (isUserExist?.isBlocked) throw new UserBlockedError();

    if (isUserExist?.isBanned) throw new UserBannedError();
