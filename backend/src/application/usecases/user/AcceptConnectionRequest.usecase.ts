...
import IAcceptConnectionRequestUsecase from '../../interfaces/usecases/user/IAcceptConnectionRequest.usecase';
...
export default class AcceptConnectionRequestUsecase implements IAcceptConnectionRequestUsecase {
  ...
  async execute(dto: AcceptConnectionRequestDTO): Promise<ConnectionRequestDTO | null> {
    ...
    if (acceptedRequest) {
      ...
      const myConnectionUpdateResult = await this._userRepo.addToConnection(
        myId as string,
        senderId as string
      );
      //update sender's connection
      const sendersConnectionUpdateResult = await this._userRepo.addToConnection(
        senderId as string,
        myId as string
      );
