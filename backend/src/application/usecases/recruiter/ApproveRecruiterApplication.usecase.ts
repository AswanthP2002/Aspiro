...
import IApproveRecruiterApplicationUsecase from '../../interfaces/usecases/recruiter/IApproveRecruiterApplication.usecase';
...
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
    ...
    try {
      ...
      if (updatedRecruiter) {
        await this._userRepo.update(updatedRecruiter.userId as string, {
          isRecruiter: true,
          role: ['user', 'recruiter'],
        }); // testing
        ...
        const personalizedBody = body.replace(
          '[Recruiter Name]',
          recruiterDetails?.userProfile.name || 'Recruiter'
        );
        if (email) {
