...
import { MyApplicationsListDTO } from '../../DTOs/jobApplication/myApplications.dto';
...
export default class JobApplicationMapper {
  ...
  public SingleJobApplicationDetailsAggregatedToDTO(
    ...
    return {
      ...
      jobDetails: {
        _id: data?.jobDetails?._id as string,
        jobTitle: data?.jobDetails?.jobTitle as string,
      },
  ...
  public jobApplicationCompanyJobRecruiterEntityToJobApplicationTrackDTO(
    data: JobApplicationCompanyRecruiterAggregated
  ): JobApplicationTrackingDTO {
    return {
      _id: data._id,
      status: data.status,
      ...
      recruiterDetails: {
        _id: data?.recruiterDetails?._id as string,
        name: data?.recruiterDetails?.fullName as string,
      },
      companyDetails: {
        _id: data?.companyDetails?._id as string,
        name: data?.companyDetails?.name as string,
      },
    };
  }
}
