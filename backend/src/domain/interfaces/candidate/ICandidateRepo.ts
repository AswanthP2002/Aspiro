import CandidateAggregated from "../../entities/candidate/candidateAggregated";
import CandidatePaginated from "../../entities/candidate/candidatePaginated";
...
export default interface ICandidateRepo extends IBaseRepo<Candidate>{
    ...
    updateVerification(email : string) : Promise<Candidate | null>
    ...
    findCandidates(search? : string, page? : number, limit? : number, sort? : string, filter? : any) : Promise<CandidatePaginated | null>
    ...
    candidateAggregatedData(candidateId : string) : Promise<CandidateAggregated | null> //need to update the aggregated data return type based on architectural correction
    addSocialLink(candidateId : string, socialLink : SocialLinks) : Promise<Candidate | null>
    getSocialLinks(candidateId : string) : Promise<any>
    deleteSocialLink(candidateId : string, domain : string) : Promise<void>
    uploadProfilePhoto(candidateId : string, cloudinaryUrl : string, cloudinaryPublicId : string) : Promise<Candidate | null>
    removeProfilePhoto(candidateId : string) : Promise<void>
    uploadCoverPhoto(candidateId : string, cloudinaryUrl : string, cloudinaryPublicId : string) : Promise<Candidate | null>
