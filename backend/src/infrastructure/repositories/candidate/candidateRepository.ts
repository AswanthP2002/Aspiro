...
export default class CandidateRepository implements CandidateRepo {
    ...
    async create(candidate: Candidate): Promise<string> {
        ...
        return `Candidate created successfully with id : ${(await result).insertedId.toHexString()}`
    }
    
    async findByEmail(email: string): Promise<Candidate | null> {
        const db = await connectDb()
        return db.collection<Candidate>(this.collection).findOne({email:email})
    }

    async findByUsername(username: string): Promise<Candidate | null> {
        const db = await connectDb()
        return db.collection<Candidate>(this.collection).findOne({username:username})
    }

    async findById(id: string): Promise<Candidate | null> {
        const db = await connectDb()
        return db.collection<Candidate>(this.collection).findOne({_id:new ObjectId(id)})
    }
}