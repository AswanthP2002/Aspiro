...
export default class EducationRepository implements IEducationRepo {
    ...
    async addEducation(education: Education): Promise<boolean> {
        ...
        const result = await db.collection<Education>(this.collection).insertOne(education)
        return result.acknowledged
    }

    async getEducations(candidateId : string): Promise<Education[]> {
        const db = await connectDb()
        const result = await db.collection<Education>(this.collection).find({candidateId:new mongoose.Types.ObjectId(candidateId)}).toArray()
        return result
    ...
    async deleteEducation(educationId: string): Promise<boolean> {
        const db = await connectDb()
        const result = await db.collection<Education>(this.collection).deleteOne({_id:new mongoose.Types.ObjectId(educationId)})
        return result.acknowledged
    }

    async editEducation(educationId: string, education: Education): Promise<boolean> {
        const db = await connectDb()
        const result = await db.collection<Education>(this.collection).updateOne(
