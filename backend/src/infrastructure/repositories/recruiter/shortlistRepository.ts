...
export default class ShortlistRepository extends BaseRepository<Shortlist> implements IShortlistRepo {
    db: Db
    collection: string
    constructor(db: Db) {
        super(db, 'shortlist')
    ...
    async getShortlistDataAggregated(jobId: string): Promise<any[]> {
        const result = await this.db.collection<Shortlist>(this.collection).aggregate([
            [
                {
