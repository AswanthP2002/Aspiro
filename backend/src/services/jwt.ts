...
export const generateRefreshToken = async (paylod: { id: string; email: string; role: string }) => {
  ...
  return jwt.sign({ id: paylod.id, email: paylod.email, role: paylod.role }, secret, {
    expiresIn: '1d',
  });
};
...
export const generateOtherSecurityToken = async (paylod: { id: string; code: string }) => {
  return jwt.sign(paylod, secret, { expiresIn: '5m' });
};
